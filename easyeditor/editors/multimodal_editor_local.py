...
class MultimodalEditor:
    ...
    def edit(self,
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                # Only needs retrieved icl_examples. 
                _, _, icl_examples = self.model, {}, self.apply_algo(
                    self.model,
                    self.tok,
                    request,
                    self.hparams,
                ...
                metrics = {
                    ...
                    "time": exec_time,
                    # request: CounterFact dataset record ???
                ...
                if 'multimodal_locality_output' in metrics['post'].keys():
                    ...
                    # locality evaluates the accuracty in the top 10 tokens with highest probabilities
                    # TODO: evaluates all tokens in the whole sentence?
    ...
    def  edit_dataset(self,
        ...
        for i, request in tqdm(enumerate(ds), desc='Editing dataset', total=len(ds)):
            ...
            if self.alg_name == 'IKE':
                ...
                pre = compute_multimodal_edit_results(self.model, self.model_name, self.hparams, self.tok, request, self.hparams.device)
                metrics["pre"] = pre
                if 'locality_output' in metrics['post'].keys():
                    assert len(metrics['post']['locality_output']) == \
                            len(metrics['pre']['locality_output'])
    ...
    def _prepare_requests(self,
        ...
        locality_inputs = locality_inputs or {}
        ...
        if "text" in locality_inputs.keys():
            ...
            assert len(locality_inputs['text']['prompt']) == len(locality_inputs['text']['ground_truth']) \
                    == len(requests), 'One Edit instance needs one locality input.....'
        ...
        if "vision" in locality_inputs.keys():
            ...
            assert len(locality_inputs['vision']['prompt']) == len(locality_inputs['vision']['ground_truth']) \
                == len(locality_inputs['vision']['image']) == len(requests), 'One Edit instance needs one locality input.....'
